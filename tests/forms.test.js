    ...
    try {
        ...
        try {
            ...
            if (formAction && formAction.includes('YOUR_FORM_ID')) {
                ...
                // Check if form uses JS-based Formspree submission (no action attr needed)
                const hasFormspreeJS = await page.evaluate(() =>
                    document.body.innerHTML.includes('formspree.io')
                );
                if (hasFormspreeJS) {
                    results.pass('Form submission configured via JS (Formspree fetch)', { action: formAction });
                } else {
